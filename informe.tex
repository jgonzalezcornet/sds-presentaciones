...
A continuación se presentan los resultados obtenidos, agrupados por escenario.

\subsection{Escenarios sin partícula líder}
\subsubsection{Evolución temporal del observable}
...
En la Fig. 1 se presenta la evolución de la polarización en función del tiempo para el escenario base sin partícula líder.

Para determinar el instante a partir del cual se considera que el sistema alcanza el estado estacionario, se aplicó un criterio empírico basado en la inspección visual de las curvas. Tomando como referencia la evolución correspondiente al máximo nivel de ruido simulado ($\eta = 5$), se identificó el punto a partir del cual el observable estabiliza sus fluctuaciones alrededor de un valor medio constante. Este mismo instante de corte se aplicó de forma fija y unificada para calcular la polarización promedio en todos los valores de ruido analizados.

\subsubsection{Relación entre el ruido del sistema y el observable}

% FIGURA 2
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{figuras/figura2.png}
	\caption{Polarización en función del ruido para el escenario base sin partícula líder}
\end{figure}

Observamos cómo para valores bajos de $\eta$, la polarización se mantiene cercana a 1, indicando un alto grado de alineamiento colectivo entre las partículas. A medida que el ruido aumenta, se observa una disminución progresiva de va, lo que refleja la pérdida de coherencia en el movimiento. Esta evolución evidencia una transición continua desde un estado ordenado hacia uno desordenado, en concordancia con el comportamiento esperado del modelo de Vicsek. Para valores elevados de $\eta$, la polarización tiende a cero, caracterizando un régimen dominado por el ruido.
...
\subsection{Escenarios con partícula líder de dirección fija}
...
\begin{figure}[H]
	...
	\caption{Evolución de la polarización con respecto al tiempo, con una partícula líder con una dirección fija.}
\end{figure}

La Fig. 3 exhibe el comportamiento temporal de la polarización para el escenario que incorpora un líder con dirección fija.

Para la determinación del inicio del estado estacionario en este escenario, se aplicó el mismo criterio empírico detallado en la Sección 5.1.1.
...
\begin{figure}[H]
	...
	\caption{Representa la polarización en función del ruido para un sistema con una partícula líder con dirección fija.}
\end{figure}

En la Fig. 4 se presenta la polarización en función del ruido para el escenario con líder de dirección fija. En todos los casos, para valores bajos de $\eta$, la polarización se mantiene cercana a 1, indicando un alto grado de alineamiento colectivo entre las partículas. A medida que el ruido aumenta, se observa una disminución progresiva de $v_a$, lo que refleja la pérdida de coherencia en el movimiento. Esta evolución evidencia una transición continua desde un estado ordenado hacia uno desordenado, en concordancia con el comportamiento esperado del modelo de Vicsek. Para valores elevados de $\eta$, la polarización tiende a cero, caracterizando un régimen dominado por el ruido. Cada punto de las curvas corresponde al promedio de 50 ejecuciones independientes, mientras que las barras de error representan el desvío entre dichas ejecuciones, proporcionando una medida de la dispersión del observable.

\subsection{Escenarios con partícula líder de trayectoria circular}
...
\begin{figure}[H]
	...
	\caption{evolución de la polarización con respecto al tiempo, con una partícula líder con una trayectoria circular.}
\end{figure}

La Fig. 5 presenta la evolución temporal del observable para el sistema con una partícula líder de trayectoria circular.

De manera análoga a los escenarios anteriores, el inicio del régimen estacionario se estableció aplicando el criterio establecido en la Sección 5.1.1.
...
En la Fig. 6 se presenta la polarización en función del ruido para el escenario con líder de trayectoria circular. Al igual que en los casos anteriores, para valores bajos de $\eta$ la polarización se mantiene cercana a 1, mientras que a medida que el ruido aumenta se observa una disminución progresiva de $v_a$, evidenciando la transición desde un estado ordenado hacia uno desordenado. Cada punto corresponde al promedio de 50 ejecuciones independientes y las barras de error representan el desvío entre dichas ejecuciones.
...
Al comparar visualmente los gráficos de evolución temporal (Fig. 1, Fig. 3 y Fig. 6), se observa que no existen diferencias significativas entre los tres escenarios. La presencia de la partícula líder, ya sea con dirección fija o circular, no altera el comportamiento general del grupo.

En todas las configuraciones, la polarización $v_a$ muestra la misma dinámica: un aumento rápido al inicio, seguido de una etapa donde se estabiliza. Como se evidencia en las curvas superpuestas, el valor medio que alcanza la polarización en el estado estacionario y la amplitud de sus fluctuaciones varían de acuerdo a la intensidad del ruido $\eta$.

Esto demuestra que, frente a una gran cantidad de partículas regulares, la introducción de un único líder con trayectoria predefinida no es suficiente para modificar el nivel de orden global ni el tiempo que tarda el sistema en estabilizarse.
...
Con el objetivo de analizar el efecto de la densidad sobre el comportamiento del sistema, se realizaron simulaciones adicionales para $\rho=2$ y $\rho=8$, considerando distintos valores del parámetro de ruido. Para el análisis detallado ilustrado en los gráficos de esta sección, se tomó como caso de estudio el escenario del sistema base sin líder.

A nivel macroscópico, los resultados obtenidos muestran un comportamiento cualitativamente similar al caso base $\rho=4$, observándose una transición desde un estado ordenado hacia uno desordenado a medida que aumenta el ruido. No obstante, se identifican diferencias en la dinámica del sistema asociadas a la densidad.

Para determinar el inicio del régimen estacionario en estos nuevos escenarios, se evaluaron las evoluciones temporales de forma independiente. A partir de un criterio empírico basado en la inspección visual de las curvas de polarización, se determinó que las fluctuaciones del sistema se estabilizan superado el transitorio inicial. Por este motivo, se estableció de forma fija el comienzo del estado estacionario a los 200 pasos temporales para todos los casos correspondientes a este análisis.

En la Fig. 8 y la Fig. 9 se observa el comportamiento de la polarización en el escenario sin partícula líder para el sistema menos denso ($\rho=2$) sometido a un ruido bajo ($\eta=0,1$) y a un ruido elevado ($\eta=5$), respectivamente. En dichos gráficos, la línea vertical intermitente marca el instante empírico de estabilización definido.

% FIGURA 8
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{figuras/figura8.png}
	\caption{Polarización vs tiempo para $\rho=2$, $\eta=0,1$ (sin líder).}
...
\begin{figure}[H]
	...
	\caption{Polarización vs tiempo para $\rho=2$, $\eta=5$ (sin líder).}
	\label{fig:densidad2_3}
\end{figure}

Por otro lado, la Fig. 10 y la Fig. 11 exhiben la evolución temporal del observable para este mismo caso sin líder al incrementar la densidad del sistema ($\rho=8$) para los mismos niveles de ruido ($\eta=0,1$ y $\eta=5$).

% FIGURA 10
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{figuras/figura10.png}
	\caption{Polarización vs tiempo para $\rho=8$, $\eta=0,1$ (sin líder).}
...
\begin{figure}[H]
	...
	\caption{Polarización vs tiempo para $\rho=8$, $\eta=5$ (sin líder).}
...
Por otro lado, el análisis de la densidad revela que su efecto es fundamental para la consolidación del movimiento cooperativo. Una mayor densidad $\rho$ favorece el orden colectivo debido a que incrementa el número promedio de partículas presentes dentro del radio de interacción $r_c$ de cada agente. Al haber una mayor cantidad de vecinos participando en el cálculo local de la dirección media, el sistema logra promediar y atenuar de manera mucho más efectiva las perturbaciones individuales introducidas por el ruido $\eta$. Como resultado, el consenso local se vuelve más robusto, permitiendo que el enjambre mantenga una polarización $v_a$ más elevada y estable para resistir los efectos del desorden.
